...
Phase 015 is not Phase 001, so the Phase 001 special Java reading list is not the controlling implementation scope for this handoff.

## 1. Phase Goal

Produce a docs-only production identity issuer/validator boundary artifact:

- Required durable artifact: `docs/harness/19-production-identity-issuer-validator-boundary.md`.
- Select or narrowly defer, at governance level only, the future production identity issuer/validator direction.
- Compare candidate identity directions:
  ...
  - deliberate deferral if no safe issuer/validator selection can be made without a broader human decision
...
- Preserve Phase 006 route freeze, Phase 012 config-store boundary, Phase 013 permission inventory and Phase 014 production auth/gateway target-scope constraints.
...
This phase is docs-only. It must not implement gateway/auth/JWT, auth-service, user-service, login/session, OAuth, SSO, external IdP integration, permission behavior changes, config mutation, tests, static guards or business code.
...
Window 4 may later update state, debt, backlog or transition lifetime after Window 3 approval. Window 2 must not do that in Phase 015.
...
- `docs/harness/09-window-protocol.md`
- `docs/harness/10-steering-state-machine.md`
- durable Phase 008 through Phase 014 artifacts
- prior phase handoffs

If satisfying Phase 015 appears to require any forbidden file, Window 2 must stop as blocked.
...
- Current inherited fact inventory from Phase 013 and Phase 014.
- Production identity issuer/validator option matrix.
- Governance-only selected or deferred identity direction.
- Future token/session semantics readiness gates.
- User profile source and validator ownership rules.
- Service principal and service-to-service identity handoff requirements.
- Audit identity field and actor classification requirements.
- Demo-header compatibility constraints.
- Gateway and route-compatibility dependencies.
...
Current belongs baseline inherited from Phase 013 and Phase 014:

| Area | Current host | Phase 015 boundary |
| --- | --- | --- |
...
| Runtime user context | `UserContextFilter`, `UserContext`, `SecurityUtils` | Runtime carrier only. Not a selected production issuer or validator. |
...
| Production gateway/auth/JWT/session/login | Not implemented | Future-only target area requiring later Window 0 selection and human approval. |

Future belongs options that may be documented only:

- A future gateway-local JWT validator may belong to an approved gateway/ingress component.
- A future auth-service issuer/validator may belong to a new auth-service only after later approval.
- A future user-service may own user profile facts, but only after a separate approved user/profile authority decision.
- A future external IdP or directory may supply identity or claims only after a later approved integration decision.
- A future service-principal validator may belong to the approved auth/gateway/service boundary, not to frontend utilities, domain read models, fallback metadata or ad hoc headers.

Phase 015 must not create any of these future hosts.
...
Phase 015 governance-only authority rules:

- The artifact must distinguish current runtime context from future trusted identity authority.
- It may select a preferred future identity issuer/validator direction, or deliberately defer selection if no bounded decision is safe.
- If a preferred direction is selected, it must be labeled future-only and not current runtime authority.
...
- Role authority is not selected in Phase 015 except where identity issuer/validator options create dependencies for the later production role authority phase.
- User profile ownership must be separated from token validation if the selected option requires that distinction.
- Service principal validation and user delegation semantics may be specified as future requirements only.
- Audit identity semantics may be specified as future requirements only and must not change current audit fields.
...
- No `role-access-configs.json` mutation or role-store migration may occur.
...
Contract language allowed in the Phase 015 artifact:

- future issuer/validator options
- future selected or deferred identity direction
- future token/session readiness prerequisites
- future user profile and validator ownership constraints
- future service principal and service-to-service identity handoff requirements
- future audit identity requirements
- future gateway compatibility and route-migration prerequisites
- future rollback and compatibility constraints

Contract language forbidden in the Phase 015 artifact:

- claiming a new current auth endpoint exists
- claiming gateway/JWT/session/login/OAuth/SSO behavior is implemented
- claiming an auth-service, user-service, external IdP or directory integration exists
...
Phase 015 is acceptable only as static governance documentation plus a Window 2 implementation handoff.

## 11. Required Production Identity Issuer/Validator Boundary Artifact Shape

`docs/harness/19-production-identity-issuer-validator-boundary.md` must include:
...
- Production identity issuer/validator option matrix.
- Governance-only selected direction, or explicit bounded deferral if selection is not safe.
- Rationale for why the selected or deferred direction fits Phase 014's backend-owned ingress/auth target.
- User profile source and validator ownership separation rules.
- Token/session semantics readiness gates.
- Service-principal validation and service-to-service identity handoff requirements.
- Audit identity requirements for human actor, delegated actor, service principal and system-triggered action.
...
Phase 015 is acceptable only if all conditions hold:

- `docs/harness/19-production-identity-issuer-validator-boundary.md` exists and is the primary durable production identity issuer/validator boundary artifact.
- `docs/harness/handoffs/phase-015-implementation.md` records exact files changed and verification outcomes.
- The artifact is docs-only and does not claim runtime implementation.
- The artifact restates current Phase 013 and Phase 014 facts for headers, defaults, runtime context, role config, backend permission checks, no-explicit-permission read surfaces, frontend gating and current absence of production auth.
- The artifact selects or explicitly defers a future production identity issuer/validator direction while labeling it future-only and not current runtime authority.
- The artifact preserves demo-header compatibility without changing current `X-User-Id` or `X-User-Role` behavior.
- The artifact defines user profile source, token/session, service-principal, service-to-service handoff and audit identity readiness gates without changing code, headers, Kafka payloads, DTOs, frontend types or runtime behavior.
...
- The artifact preserves Phase 005 through Phase 014 constraints.
...
- No Java, Python, frontend, database, Redis, Kafka, `ai-config`, prompt-template, dependency, build-config, deployment, state, debt, backlog, transition lifetime, durable Phase 008/009/010/011/012/013/014 artifact or prior handoff file changes occur.
...
- `git status --short --untracked-files=all` after Window 2 shows only allowed Phase 015 documentation files as new Window 2 changes, aside from pre-existing unrelated dirty files clearly excluded from the Window 2 claim.
...
Expected result: the durable artifact and implementation handoff contain the required identity boundary facts, future-only labels and inherited guardrail references.
...
Expected result: Window 2 records the final dirty/untracked state and claims only allowed Phase 015 documentation files, excluding pre-existing unrelated files.

Maven, npm build and Python runtime verification are not required because Phase 015 forbids Java, frontend, Python and test-code changes. If Window 2 touches any such area, it must stop as blocked instead of validating with builds.

## 14. Blocker Stop Rules

Window 2 must stop and write a blocker in `docs/harness/handoffs/phase-015-implementation.md` if any of these become necessary:
...
- Needing code behavior changes to make the identity issuer/validator boundary artifact true.
- Finding that identity issuer/validator selection cannot be scoped without a broader human decision than Phase 015 approved.
...
- No config mutation.
- Window 2 may perform docs-only production identity issuer/validator boundary work inside the allowed file boundaries above.
