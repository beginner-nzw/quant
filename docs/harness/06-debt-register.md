...
| --- | --- | --- | --- | --- | --- | --- |
| D001 | High | Service boundary | `ai-orchestration-service` hosts too many domains | Phase 001 split controller classes; Phase 002 split internal query service ownership; Phase 005 selected next-governance-horizon modular monolith; Phase 008 documented per-domain transition-host exit criteria and readiness gates; Phase 009 documented report-specific readiness gates and blockers, but the service host still contains multiple domains | Use the Phase 008 inventory and Phase 009 report readiness artifact before any extraction, route migration or permanence claim; select follow-up domain readiness or ownership work only through Window 0 and human approval | Open - Phase 009 refined report readiness, transition-host debt remains |
| D002 | High | Contract | Non-task domains exposed under `/api/tasks/*` | risk, strategy, market, report, config APIs keep legacy `/api/tasks/*` paths in split controllers; Phase 006 froze the approved transitional inventory with focused backend contract tests; Phase 009 recorded the stable report URL/API inventory without migration | Later introduce domain URL groups only with explicit breaking-change approval; until then keep the Phase 006 inventory, Phase 009 report contract inventory and guards in sync with any approved contract change | Open - report contracts documented in Phase 009, legacy namespace remains transition debt |
| D003 | Medium | Authority | Future `research-workbench` or fallback metadata surfaces can be mistaken as SoT | Phase 003 added Java backend display-only contract notes and boundary tests that keep workbench out of command/projection authority and guard it from domain writes; Phase 004 added Python fallback provenance for planner, intent, financial, risk, report and market fallback paths; Phase 007 added frontend authority notes and a focused static guard for current workbench/fallback consumers; Phase 009 documented report fallback provenance and frontend display fields as non-authoritative | Keep fallback/workbench metadata display-only, extend equivalent guardrails to future surfaces, and do not promote provenance into command or projection authority | Open - current known backend, Python, frontend and report readiness surfaces mitigated |
...
Next work should use the Phase 008 inventory and Phase 009 report readiness artifact to score bounded follow-up phases, such as market/data-ingest ownership, risk/strategy projection ownership, auth/gateway, legacy route migration, config-store decisions or report extraction/route planning only if explicitly selected by Window 0 and approved by the user.
