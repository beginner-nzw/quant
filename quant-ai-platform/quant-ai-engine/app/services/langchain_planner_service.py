...
class PlannerOutput(BaseModel):
    ...
    sourceReportAvailable: bool = False
    marketSnapshotReady: bool = False
    executionFocus: list[str] = Field(default_factory=list)


class LangChainPlannerService:
    SCENARIO_CODE = "TASK_PLANNING"

    def __init__(self):
        self.factory = LangChainLlmFactory()
        self.prompt_builder_service = PromptBuilderService()
        self.parser = (
            PydanticOutputParser(pydantic_object=PlannerOutput)
    ...
    def generate_plan_result(
        ...
        *,
        ...
        system_prompt, user_prompt = self.prompt_builder_service.build_planner_prompts(
            state=state,
        ...
        try:
            ...
            log_error(
                state.get("trace_id", ""),
                f"[AI-ENGINE][LANGCHAIN] planner generation failed err={exc}",
            )
            return None

        if not result.executionFocus:
            log_error(
                state.get("trace_id", ""),
                "[AI-ENGINE][LANGCHAIN] planner generation returned empty execution focus",
            )
            return None

        log_info(
            state.get("trace_id", ""),
            f"[AI-ENGINE][LANGCHAIN] planner generated model={self.factory.model_name(self.SCENARIO_CODE)}",
        )
