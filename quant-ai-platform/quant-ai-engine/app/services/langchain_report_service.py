...
class LangChainReportService:
    SCENARIO_CODE = "REPORT_GENERATION"

    def __init__(self):
        self.factory = LangChainLlmFactory()
        self.prompt_builder_service = PromptBuilderService()
        self.parser = (
            PydanticOutputParser(pydantic_object=ReportGenerationOutput)
    ...
    def generate_report(
        ...
        *,
        ...
        try:
            ...
            log_error(
                state.get("trace_id", ""),
                f"[AI-ENGINE][LANGCHAIN] report generation failed err={exc}",
            )
            return None
        ...
        if not result.summary.strip():
            log_error(
                state.get("trace_id", ""),
                "[AI-ENGINE][LANGCHAIN] report generation returned empty summary",
            )
            return None

        log_info(
            state.get("trace_id", ""),
            f"[AI-ENGINE][LANGCHAIN] report generated model={self.factory.model_name(self.SCENARIO_CODE)}",
        )
