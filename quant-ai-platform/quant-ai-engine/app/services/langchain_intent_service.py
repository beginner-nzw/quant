...
class IntentOutput(BaseModel):
    analysisMode: str = ""
    focusDimensions: list[str] = Field(default_factory=list)
    sourceDomain: str = "UNKNOWN"
    contextSource: str | None = None
    sourceReportId: str | None = None
    latestInsightReportId: str | None = None
    pendingReviewCount: int = 0
    reviewPressure: str = "LOW"


class LangChainIntentService:
    SCENARIO_CODE = "TASK_INTENT"

    def __init__(self):
        self.factory = LangChainLlmFactory()
        self.prompt_builder_service = PromptBuilderService()
        self.parser = (
            PydanticOutputParser(pydantic_object=IntentOutput)
    ...
    def generate_intent_result(
        ...
        *,
        ...
        system_prompt, user_prompt = self.prompt_builder_service.build_intent_prompts(
            state=state,
        ...
        try:
            ...
            log_error(
                state.get("trace_id", ""),
                f"[AI-ENGINE][LANGCHAIN] intent generation failed err={exc}",
            )
            return None

        if not result.focusDimensions:
            log_error(
                state.get("trace_id", ""),
                "[AI-ENGINE][LANGCHAIN] intent generation returned empty focus dimensions",
            )
            return None

        log_info(
            state.get("trace_id", ""),
            f"[AI-ENGINE][LANGCHAIN] intent generated model={self.factory.model_name(self.SCENARIO_CODE)}",
        )
