...
public class StrategySignalRecordManager {
    ...
    private final StrategySignalFollowUpSummaryManager followUpManager;
    ...
    private List<StrategySignalListItemVO> listStrategySignalRecordsFromDomain(List<StrategySignalDO> signals,
        ...
        List<RiskWarningTaskProjection> followUpTasks = readManager.loadStrategySignalFollowUpTasks();
        ...
        return signals.stream()
                .map(signal -> {
                    RiskWarningTaskProjection task = taskMap.get(signal.getTaskId());
                    TaskReportProjection report = reportMap.get(signal.getTaskId());
                    TaskReportRiskProjection warning = riskWarningMap.get(signal.getTaskId());
    ...
    private List<StrategySignalListItemVO> listStrategySignalRecordsFromReports(StrategySignalPageQueryDTO queryDTO,
        ...
        return reports.stream()
                .map(report -> {
                    RiskWarningTaskProjection task = taskMap.get(report.taskId());
