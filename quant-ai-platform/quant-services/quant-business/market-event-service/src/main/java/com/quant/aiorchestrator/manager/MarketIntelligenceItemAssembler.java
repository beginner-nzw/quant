...
public class MarketIntelligenceItemAssembler {
    ...
    private final MarketStrategySignalRuleManager ruleManager;

    public MarketIntelligenceItemAssembler(MarketStrategySignalRuleManager ruleManager) {
        this.ruleManager = ruleManager;
    }

    public MarketIntelligenceListItemVO toMarketIntelligenceItem(TaskReportProjection report,
                                                                 MarketEventTaskProjection task,
                                                                 MarketEventFollowUpTaskSummaryManager.FollowUpSummary followUpSummary,
                                                                 TaskReportRiskProjection warning,
                                                                 List<TaskReportRiskDetailProjection> warningDetails,
                                                                 StrategySignalReadProjection strategySignal) {
        if (report == null || task == null) {
            return null;
        }

        String summary = ruleManager.resolveDisplaySummary(report.revisedSummary(), report.summary());
        String reportType = ruleManager.resolveReportType(report, task);
        MarketStrategySignalRuleManager.RiskProjection riskProjection = ruleManager.resolveRiskProjection(
                report,
                warning,
                warningDetails
        );
        ...
        Double reportConfidenceScore = report.confidenceScore() == null ? null : report.confidenceScore().doubleValue();
        ...
        if ((summary == null || summary.isBlank())
                && (report.resultRef() == null || report.resultRef().isBlank())
                && (reportType == null || reportType.isBlank())) {
            return null;
        }

        ReportReviewStatusEnum reviewStatus = ruleManager.resolveReviewStatus(report.reviewStatus());
        ...
        vo.setSourceChannel(task.sourceDomain());
    ...
    private List<String> buildMarketIntelligenceSourceTags(MarketEventTaskProjection task,
        ...
        if (task != null && task.sourceDomain() != null && !task.sourceDomain().isBlank()) {
