...
const text = {
  ...
  workbench: '投研工作台',
  createTask: '发起研究',
  loadStatsFailed: '已驳回统计加载失败',
  loadStatsError: '已驳回统计加载异常',
  loadFailed: '已驳回报告加载失败',
  loadError: '已驳回报告加载异常'
} as const
    ...
    <el-card style="margin-top: 16px;">
      <el-space>
        <el-button
          ...
          :key="tab.route"
          :type="tab.route === '/reports/rejected' ? 'warning' : 'default'"
    ...
    <el-card style="margin-top: 16px;">
      <el-table :data="pageData.records" v-loading="loading" border :empty-text="text.empty">
        ...
        <el-table-column :label="text.reviewStatus" width="120">
          <template #default="{ row }">
            <el-tag :type="getReviewStatusTagType(row.reportReviewStatus || REPORT_REVIEW_STATUS.REJECTED)">
              {{ getReviewStatusText(row.reportReviewStatus || REPORT_REVIEW_STATUS.REJECTED) }}
            </el-tag>
          </template>
        </el-table-column>
